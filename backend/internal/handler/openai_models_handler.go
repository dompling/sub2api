...
func (h *GatewayHandler) pinnedOpenAIModels(c *gin.Context, group *service.Group) {
	...
	keyRestricted := apiKey != nil && len(apiKey.AllowedModels) > 0
	etag := c.GetHeader("If-None-Match")
	if c.Param("model") != "" || keyRestricted {
		// The group catalog cannot validate a key-filtered or single-model response.
		etag = ""
	}
	...
	if keyRestricted {
		body, err := filterOpenAIModelsByKey(response.Body, apiKey)
		if err != nil {
...
func filterOpenAIModelsByKey(body []byte, apiKey *service.APIKey) ([]byte, error) {
	var catalog struct {
	...
	models := make([]json.RawMessage, 0, len(catalog.Data))
	for _, raw := range catalog.Data {
		...
		if apiKey.IsModelAllowed(model.ID) {
