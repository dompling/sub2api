...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if platform == service.PlatformComposite {
		...
		if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled() {
			...
			models := apiKey.Group.ModelAllowlist.FilterForListing(source)
			writeAllowlistedModelsList(c, service.PlatformComposite, filterModelsByKeyWhitelist(models, apiKey))
			return
		}
	...
	if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled() {
		...
		models := filterModelsByGroupAllowlist(platform, source, apiKey.Group.ModelAllowlist)
		writeAllowlistedModelsList(c, platform, filterModelsByKeyWhitelist(models, apiKey))
		return
	}
	...
	if len(defaultFallback) > 0 || (apiKey != nil && len(apiKey.AllowedModels) > 0) {
...
// CodeBuddy 的候选模型来自实时配置，显式选择的新模型不能被旧账号快照过滤。
// 通配条目仍只展开可用模型，避免把通配符本身返回给客户端。
func filterModelsByGroupAllowlist(platform string, source []string, allowlist service.GroupModelAllowlist) []string {
	if platform == service.PlatformCodeBuddy && allowlist.Enabled {
		source = append([]string(nil), source...)
		for _, model := range allowlist.Models {
			if !strings.HasSuffix(strings.TrimSpace(model), "*") {
				source = append(source, model)
			}
		}
	}
	return allowlist.FilterForListing(source)
}
...
func (h *GatewayHandler) codexModelIDsForGroup(ctx context.Context, group *service.Group, platformOverride string) []string {
	...
	if group.ModelAllowlistEnabled() {
		return filterModelsByGroupAllowlist(platform, modelListingSource(platform, availableModels, fallbackModels), group.ModelAllowlist)
	}
	if len(availableModels) > 0 {
		return availableModels
	}
