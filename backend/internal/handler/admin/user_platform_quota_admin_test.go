...
func TestUpdateUserPlatformQuotas_CodeBuddyAndNewUpstreamPlatforms(t *testing.T) {
	for _, platform := range []string{"codebuddy", "adobe", "minimax", "opencode_go"} {
		t.Run(platform, func(t *testing.T) {
			repo := &upsertCapturingQuotaRepo{}
			h := buildTestHandler(repo, &billingCacheStub{})
			c, w := putReq(t, `{"quotas":[{"platform":"`+platform+`","daily_limit_usd":0}]}`)
			h.UpdateUserPlatformQuotas(c)
			if w.Code != http.StatusOK {
				t.Fatalf("expected 200, got %d: %s", w.Code, w.Body.String())
			}
			if len(repo.upsertCalls) != 1 || len(repo.upsertCalls[0].records) != 1 {
				t.Fatalf("explicit zero quota must be persisted: %+v", repo.upsertCalls)
			}
			r := repo.upsertCalls[0].records[0]
			if r.Platform != platform || r.DailyLimitUSD == nil || *r.DailyLimitUSD != 0 {
				t.Fatalf("unexpected quota: %+v", r)
			}
		})
	}
}
