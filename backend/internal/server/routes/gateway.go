...
func RegisterGatewayRoutes(
	...
	requireAllowedModels := middleware.RequireAllowedModels()

	isOpenAIResponsesCompatibleGatewayPlatform := func(c *gin.Context) bool {
		switch getGroupPlatform(c) {
		case service.PlatformOpenAI, service.PlatformGrok,
			service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseek,
			service.PlatformMiniMax, service.PlatformOpenCodeGo, service.PlatformCodeBuddy:
	...
	countTokensHandler := func(c *gin.Context) {
		switch getGroupPlatform(c) {
		case service.PlatformOpenAI, service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseek, service.PlatformMiniMax, service.PlatformOpenCodeGo, service.PlatformCodeBuddy:
	...
	gateway.Use(requireAllowedModels)
	...
	gemini.Use(requireAllowedModels)
	...
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), requireAllowedModels, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), requireAllowedModels, groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
	...
	antigravityV1.Use(requireAllowedModels)
	...
	antigravityV1Beta.Use(requireAllowedModels)
