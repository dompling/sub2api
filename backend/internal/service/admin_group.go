...
func defaultModelsListCandidateIDs(platform string) []string {
	...
	case PlatformAdobe:
		// 只给图像族：视频还没接线（见 adobe.ImageModelIDs 的注释）。
...
func defaultAllowImageGenerationForPlatform(platform string) bool {
	...
	//
	// Adobe groups exist solely to serve image generation, so they must default
	// enabled for the same reason: a false zero value from an older client would
	// otherwise make every request fail the gate.
...
func compositeDefaultModelsListCandidateIDs() []string {
	seen := make(map[string]struct{})
	ids := make([]string, 0)
	for _, platform := range []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformKiro, PlatformGrok, PlatformAdobe, PlatformKimi, PlatformZhipu, PlatformDeepseek, PlatformMiniMax, PlatformOpenCodeGo, PlatformCodeBuddy} {
