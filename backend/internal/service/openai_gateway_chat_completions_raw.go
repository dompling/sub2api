...
func (s *OpenAIGatewayService) forwardAsRawChatCompletions(
	...
	// 按平台记录真实上游端点：CodeBuddy 为 /v2/chat/completions，其余为 /v1/chat/completions。
	...
	// CodeBuddy 上游不支持非流式请求，即使客户端未请求流式也必须以流式发出。
...
func (s *OpenAIGatewayService) bufferRawChatCompletionsViaStream(
	...
	requestID := resp.Header.Get("x-request-id")
	aggregated, scan := s.aggregateCCStream(c, resp, "openai chat_completions raw (buffered stream)", requestID, startTime, upstreamModel)
	if scan.Err != nil {
