...
func (s *OpenAIGatewayService) forwardResponsesViaRawChatCompletions(
	...
	if clientStream {
		...
		result, err = s.bufferChatCompletionsAsResponsesViaStream(c, resp, originalModel, customTools, functionTools, toolSearch, namespaceTools, billingModel, upstreamModel, reasoningEffort, serviceTier, startTime)
	} else {
...
func (s *OpenAIGatewayService) bufferChatCompletionsAsResponsesViaStream(
	...
	aggregated, scan := s.aggregateCCStream(c, resp, "openai responses chat fallback (buffered stream)", requestID, startTime, upstreamModel)
	if scan.Err != nil {
	...
	responsesResp := apicompat.ChatCompletionsResponseToResponses(aggregated, originalModel, customTools, functionTools, toolSearch, namespaceTools)
